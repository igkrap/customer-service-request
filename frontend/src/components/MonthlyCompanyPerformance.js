...
const getYearMonthFromDate = (value) => {
...
function MonthlyCompanyPerformance() {
    ...
    const fetchReport = async () => {
      ...
      try {
        ...
        requestsResponse.data.forEach((request) => {
          ...
          const requestYearMonth = getYearMonthFromDate(candidateDate);
          if (requestYearMonth !== selectedMonth) {
            return;
          }
        ...
        setError(`월간 회사별 실적을 불러오는 중 오류가 발생했습니다: ${err.message}`);
      } finally {
        setLoading(false);
      }
    };
...
export default MonthlyCompanyPerformance;
